...
identificadores_comunes = [
    ...
    "Playing_Pilgrims", "ACT_I", "I", "Chapter_I", "chapter-1", '#link2HCH0001'
]
...
def buscar_parrafo(soup, identificador, min_caracteres=100):
    """
    Busca el primer párrafo asociado a un identificador dado y maneja casos especiales como la clase 'poem'.
    
    Args:
        soup (BeautifulSoup): El objeto BeautifulSoup que contiene el HTML del libro.
        identificador (str): El identificador del elemento HTML a buscar (por ejemplo, un ID de capítulo).
    ...
    # Buscar el elemento con el identificador
...
def obtener_primer_parrafo_libro(url):
    ...
    try:
        ...
        # 1️⃣ Primero, buscar por identificadores comunes
        resultado = None
        for id_val in identificadores_comunes:
            resultado = buscar_parrafo(soup, id_val)
            if resultado:
                break
